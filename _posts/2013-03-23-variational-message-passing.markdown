...
![贝叶斯网络](/img/posts/variational_message_passing/vmp-1.png)
图-1 贝叶斯网络 (马尔科夫毯) 
...
$$\left\langle u(X) \right\rangle _{P(X\|\phi )} = - \frac{d\tilde g(\phi )}{d\phi}$$ (1)
...
![马尔科夫毯](/img/posts/variational_message_passing/vmp-2.png)
图-2 马尔科夫毯
...
$$\sum\limits_{k \in c{h_Y}} \left\langle \phi_{XY}{(X,c{p_Y})^T}u_Y(Y) + \lambda (X,c{p_Y}) \right\rangle_{\sim Q(Y)} + const. $$

$${\rm{ = }} {\left[ {\langle \phi_Y{(p{a_Y})} \rangle}_{\sim Q(Y)} + \sum\limits_{k \in ch_Y} {\langle \phi_{XY{(X,cp_Y)}} \rangle }_{\sim Q(Y)} \right]^T}{u_Y(Y)} + {f_Y}(Y) + const.$$

$${\rm{ = }}{\left[ {\phi_Y^*} \right]^T}u_Y(Y) + {f_Y}(Y) + const.$$

其中，$$\phi_Y^* = \left\langle {\phi_Y{(p{a_Y})^T}} \right\rangle _{\sim Q(Y)} + \sum\limits_{k \in c{h_Y}} \left\langle {\phi_{XY}{(X,c{p_Y})^T}} \right\rangle_{\sim Q(Y)}$$      (5)
...
$$L\left( Q \right)$$被分解为每一个节点上的贡献值$$\left\{ {{L_i}} \right\}$$，如节点$${H_j}$$的贡献值为

$${L_j} = \left\langle {\ln P({H_i}\|p{a_j})} \right\rangle - \left\langle {\ln {Q_i}({H_i})} \right\rangle$$

$$\rm{ = } \langle {\phi _j}(p{a_j})^T \rangle \langle u_j(H_j) \rangle + \langle {f_j}(H_j) \rangle $$+$$ \langle g_j(p{a_j}) \rangle - \left[ {\phi _j^*}^T \langle u_j(H_j) \rangle + \langle {f_j}(H_j) \rangle + \tilde{g}_j(\phi _j^*) \right]$$

$${\rm{ = }}{\left( {\langle {\phi_j(p{a_j})} \rangle - \phi _j^*} \right)^T}\langle {u_j({H_j})} \rangle + \langle {g_j(p{a_j})} \rangle - \tilde{g}_j(\phi _j^*)$$
...
$${L_k} = \left\langle {\ln P({V_k}\|p{a_k})} \right\rangle$$

$${\rm{ = }}{\left\langle {\phi_j(p{a_j})} \right\rangle ^T}{u_k}({V_k}) + {f_k}({V_k}) + {\tilde g_k}\left( {\left\langle {\phi_j(p{a_j})} \right\rangle } \right)$$
...
$$m_{Y \to X} = \left\langle {u_Y}\right\rangle .$$   (6)

消息传播给父节点 (Message to parents) ：依赖于X之前从Y的co-parents接收到的消息；对任何节点A，如果A是被观测量，那么$$\left\langle {u_A} \right\rangle = u_A$$,

$$m_{X \to Y} = \tilde{\phi}_{XY}\left( \left\langle u_X \right\rangle ,{\left\{ m_{i \to X} \right\}}_{i \in c{p_Y}} \right)$$   (7)

用Y接收来自父节点与子节点的所有消息来计算$$\phi_Y^*$$，然后我们就能通过计算更新后的自然参数向量$$\phi_Y^*$$来找到Y的更新后的后验分布$$Q_Y^*$$，$$\phi_Y^*$$的计算公式如下，

$$\phi_Y^* = \tilde{\phi_Y}\left( \left\{ m_{i \to Y} \right\}_{i \in p{a_Y}} \right) + \sum\limits_{j \in c{h_Y}} m_{j \to Y} $$.   (8)

该式与 (5) 式一致。从 (1) 式可以看出自然统计量的期望$${\left\langle {u_Y} \right\rangle _{Q_Y^*}}$$是$$Q_Y^*$$的单一函数，这样我们就可以用它来计算期望的新值。变分消息传播算法通过迭代的消息传播来最优化变分分布Q.
...
*   从父节点和子节点接收 (6),(7)式所定义的消息。前提是子节点已经从$${X_j}$$的co-parents接收到消息。
*   使用 (8) 式更新自然参数向量$$\phi _j^*$$；
*   根据新的参数向量更新距向量$$\left\langle {u_j({X_j})} \right\rangle $$；
...
$${\rm{ = }}{\phi _X}{(\lambda ,\{ \theta_k\} )^T}u_X(X) + {f_X}(X) + {\tilde g_X}({\phi _X}(\lambda ,\{ \theta_k\} ))$$
